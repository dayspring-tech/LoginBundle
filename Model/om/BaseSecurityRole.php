...
abstract class BaseSecurityRole extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Dayspring\\LoginBundle\\Model\\SecurityRolePeer';
    ...
    public function setRoleName($v)
    {
        ...
        if ($this->role_name !== $v) {
            $this->role_name = $v;
            $this->modifiedColumns[] = SecurityRolePeer::ROLE_NAME;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 2; // 2 = SecurityRolePeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = SecurityRoleQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(SecurityRolePeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(SecurityRolePeer::ROLE_NAME)) {
    ...
    {
        if (isset($alreadyDumpedObjects['SecurityRole'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['SecurityRole'][$this->getPrimaryKey()] = true;
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SecurityRolePeer::ROLE_NAME)) $criteria->add(SecurityRolePeer::ROLE_NAME, $this->role_name);
     ...
     * If this SecurityRole is new, it will return
    ...
    public function setRoleUsers(PropelCollection $roleUsers, PropelPDO $con = null)
    {
        ...
        foreach ($roleUsersToDelete as $roleUserRemoved) {
            $roleUserRemoved->setSecurityRole(null);
        }
     ...
     * @return PropelObjectCollection|RoleUser[] List of RoleUser objects
     */
    public function getRoleUsersJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = RoleUserQuery::create(null, $criteria);
     ...
     *
     * @param  User $user The RoleUser object to relate
     * @return SecurityRole The current object (for fluent API support)
     */
    ...
    protected function doAddUser(User $user)
    {
        ...
        if (!$user->getSecurityRoles()->contains($this)) { $roleUser = new RoleUser();
            $roleUser->setUser($user);
            $this->addRoleUser($roleUser);

            $foreignCollection = $user->getSecurityRoles();
    ...
    {
        return (string) $this->exportTo(SecurityRolePeer::DEFAULT_STRING_FORMAT);
    }
